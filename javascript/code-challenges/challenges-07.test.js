...
const sortStarWarsCharacters = (starWarsArr) => {
  //Solution code here
  const sorted = starWarsArr.map((element) => element).sort((a, b) => parseInt(b.height) - parseInt(a.height));
  return sorted;
};
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    let end = ingredient.indexOf(' ', 3);
    result.push(ingredient.slice(end + 1));
  }
  return result;
};
// const listFoods = (recipe) => {
...
//     let start = ingredient.indexOf(' ');
//     let end = ingredient.lastIndexOf(' ');
//     result.push(ingredient.slice(start, end));
//   }
//   return result;
// };
