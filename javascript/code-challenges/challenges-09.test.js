...
const updateNumbers = (obj) => {
  const newArray = Object.entries(obj).map(([name, number]) => `${name}: ${number}`);
  return newArray;
  // let newArr = [];
  // Object.entries(obj).map((nam,num)=>{
  //   newArr.push(nam,num);
  // });
  // return newArr;
  // Solution code here...
...
const getHouses = (arr) => {
  let houses = [];
  let newValue = Object.values(arr);
  newValue.map((element) => {
    return houses.push(element.house);
  });
  return houses;
};
/*------------------------------------------------------------------------------------------------
...
const hasChildrenValues = (arr, character) => {
  const children = [];
  for (const element of Object.values(arr)) {
