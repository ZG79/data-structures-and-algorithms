...
const count = (target, input) => {
  ...
  const find = input.reduce((acc, idx) => {
    let repeat = idx.filter((element)=> element === target).length;
    return acc + repeat;
  },0);
  return find;
  // Solution code here...
...
const totalSum = (input) => {
  const combinedArrays = [].concat(...input);
  console.log(combinedArrays);
  const total = combinedArrays.reduce((acc, idx) =>{
    return acc +idx;
  });
  return total;
  // Solution code here...
...
const divisibleByFiveTwoToThePower = (input) => {
  let array = input.map((element) =>
    element.filter((el) => typeof el === 'number' && el % 5 === 0).map((idx) => Math.pow(2, idx))
  );
  return array;
};
...
function findMaleAndFemale(data) {
